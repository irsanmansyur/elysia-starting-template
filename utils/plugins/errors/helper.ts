...
export abstract class ErrorHanlder {
	...
	static validation(errorValidationString: string, path: string, method: string) {
		try {
			const parsedError = JSON.parse(errorValidationString) as APP.ValidationResponse;
			const formattedErrors = parsedError.errors.reduce((acc: Record<string, string[]>, err) => {
